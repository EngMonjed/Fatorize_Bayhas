...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['_action'])) {
    ...
    try {
        ...
        elseif ($act === 'save_invoice') {
            ...
            $saveAs      = $_POST['save_as'] ?? 'draft'; // draft or confirmed
            ...
            // حفظ البنود — كل سطر يمثل كروب×لون ويحوي عدة variants
            foreach ($rows as $r) {
                $qty       = (float)$r['qty'];
                ...
                $variantIds= $r['variant_ids'] ?? [$r['variant_id']];
                ...
                foreach ($variantIds as $variantId) {
                    ...
                    // تحديث المخزون فوراً إذا confirmed
                    if ($saveAs === 'confirmed') {
                        $pdo->prepare("INSERT INTO `{$TWI}` (warehouse_id,variant_id,product_id,quantity,min_quantity)
                            VALUES (?,?,?,?,0)
                            ON DUPLICATE KEY UPDATE quantity=quantity+?")
                            ->execute([$whId,$variantId,(int)$r['product_id'],$qty,$qty]);
                    }
                }
            }

            // تحديث حالة الفاتورة إذا confirmed
            if ($saveAs === 'confirmed') {
                $pdo->prepare("UPDATE `{$TP}` SET status='received' WHERE id=?")
                    ->execute([$purId]);
            }
...
<div class="card-sec" style="position:sticky;top:80px">
    ...
    <div class="card-sec-body">
        ...
        <div class="d-grid gap-2">
            ...
            <button class="btn btn-sm fw-600"
                    style="border-radius:9px;background:#f1f5f9;color:#475569;padding:8px"
...
function openSelectModal(items){
    ...
    items.forEach(it => {
        ...
        // تجميع بـ (size_group = selling_price)
        const gk = it.selling_price || '0';
        if (!grouped[pid].grps[gk]) grouped[pid].grps[gk] = {price:it.selling_price, age_type:it.age_type, variants:[]};
    ...
    const rows = {};
    items.forEach(it => {
        const key = `${it.product_id}_${it.selling_price}_${it.color_id||0}`;
        if (!rows[key]) {
            rows[key] = {
                key,
                ...
                variants:     [],  // كل variants هذا الكروب×لون
                sizes:        [],
                cost_price:   it.cost_price,
            };
        }
        rows[key].variants.push(it);
        rows[key].sizes.push(it.size);
    });
    ...
    sortedRows.forEach(r => {
        ...
        html += `<tr id="selrow_${r.key}" onclick="toggleSelRow(this,'${r.key}')">
            ...
            <td>${grpLabel} <small style="color:#64748b;font-size:.68rem">${r.age_type||''}</small></td>
            <td style="font-size:.8rem;color:#1e293b;font-weight:600">${sizesStr}</td>
...
function makeGrpKey(item){
    return `${item.product_id}_${item.selling_price||item.unit_price||0}_${item.color_id||0}`;
}
...
function addLine(item){
    ...
    const grpBadge=`<span style="background:${bg};color:${clr};border:1px solid ${br};border-radius:12px;font-size:.68rem;padding:2px 8px;font-weight:600">
        كروب ${grpIdx+1} — ${line.selling_price.toFixed(2)} ${symCur}
    </span>`;
