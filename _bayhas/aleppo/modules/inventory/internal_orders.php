 ...
 * المسار: /bayhas/aleppo/modules/inventory/internal_orders.php
 */
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['_action'])) {
    ...
    try {
        ...
        if ($act === 'get_factory_products') {
            if (!$factorySuffix) throw new Exception('الفرع غير مرتبط بمعمل');
        ...
        elseif ($act === 'create_order') {
            ...
            if (!$factoryId) throw new Exception('الفرع غير مرتبط بمعمل — تحقق من إعدادات الفرع');

            $items    = json_decode($_POST['items'] ?? '[]', true);
            if (empty($items)) throw new Exception('أضف بنداً واحداً على الأقل');
        ...
        elseif ($act === 'convert_to_purchase') {
            ...
            $order = $ord->fetch();
            if (!$order) throw new Exception('الطلب غير موجود أو غير مصرح');
            ...
            if (empty($approved_items)) throw new Exception('لا توجد بنود معتمدة للتحويل');

            $total = array_sum(array_column($approved_items, 'total_price'));

            // فاتورة الشراء في فرع حلب (الفرع الطالب)
            ...
            $pdo->prepare("INSERT INTO `{$TP_buy}`
                ...
                ->execute([
                    $purch_num, $total, $total,
                    $order['exchange_rate'] > 0 ? round($total/$order['exchange_rate'],4) : $total,
                    $order['currency'], $order['exchange_rate'],
                    'من طلب داخلي: '.$order['order_number'],
                    $_SESSION['user_id'],
                ]);
            ...
            foreach ($approved_items as $item) {
                $pdo->prepare("INSERT INTO `{$TPI_buy}`
                    ...
                    ->execute([
                        ...
                        $item['quantity_approved'], $item['unit_price'], $item['unit_price_usd'],
                        $item['total_price'],
                    ]);
            }
            ...
            echo json_encode(['ok'=>true,'msg'=>'تم تحويل الطلب لفاتورة شراء رقم '.$purch_num,'purchase_id'=>$purchId]);
        }
...
.ctrl-bar{background:#fff;border-radius:14px;border:1px solid #e2e8f0;padding:.65rem 1.1rem;margin-bottom:1.1rem;display:flex;align-items:center;gap:.65rem;flex-wrap:wrap}
.tab-wrap{display:flex;gap:3px;background:#f1f5f9;border-radius:9px;padding:3px}
.t-btn{padding:.38rem .9rem;border-radius:7px;font-size:.81rem;cursor:pointer;color:#64748b;border:none;background:none;font-family:'Cairo',sans-serif;font-weight:600;transition:all .15s;white-space:nowrap}
.t-btn.act{background:#fff;color:#1e3a8a;box-shadow:0 1px 4px rgba(0,0,0,.07)}
.sec-card{background:#fff;border-radius:14px;border:1px solid #e2e8f0;overflow:hidden;margin-bottom:1.1rem}
.sec-card table{margin:0;font-size:.82rem}
.sec-card th{background:#f8fafc;color:#64748b;font-size:.75rem;font-weight:600;border:none;padding:.6rem .9rem;white-space:nowrap}
.sec-card td{padding:.55rem .9rem;vertical-align:middle;border-top:1px solid #f1f5f9}
.sec-card tbody tr:hover td{background:#f8fafc}
.sec-hdr{padding:.7rem 1.1rem;border-bottom:1px solid #f1f5f9;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.5rem}
.act-btn{width:28px;height:28px;border-radius:7px;display:inline-flex;align-items:center;justify-content:center;border:1px solid #e2e8f0;background:#fff;color:#64748b;cursor:pointer;font-size:.85rem;transition:all .15s}
.act-btn:hover{background:#eff6ff;color:#2563eb;border-color:#bfdbfe}
.act-btn.danger:hover{background:#fef2f2;color:#dc2626;border-color:#fca5a5}
.act-btn.success:hover{background:#f0fdf4;color:#16a34a;border-color:#bbf7d0}
/* مودال */
.modal-content{border-radius:16px;border:none}
.mhdr-out{background:linear-gradient(135deg,#1e3a8a,#2563eb);border-radius:16px 16px 0 0;border:none}
.mhdr-in {background:linear-gradient(135deg,#065f46,#16a34a);border-radius:16px 16px 0 0;border:none}
.mhdr-view{background:linear-gradient(135deg,#4c1d95,#7c3aed);border-radius:16px 16px 0 0;border:none}
/* بناء الطلب */
.order-item-row{background:#f8fafc;border-radius:10px;border:1px solid #e2e8f0;padding:.6rem .85rem;margin-bottom:6px;display:grid;grid-template-columns:1fr auto auto auto auto;gap:.5rem;align-items:center}
.order-item-row .prod-lbl{font-size:.82rem;font-weight:600}
.order-item-row .prod-sub{font-size:.73rem;color:#64748b}
.qty-inp{width:70px;border-radius:7px;border:1px solid #e2e8f0;padding:3px 6px;font-size:.82rem;text-align:center;font-family:'Cairo',sans-serif}
.price-inp{width:90px;border-radius:7px;border:1px solid #e2e8f0;padding:3px 6px;font-size:.82rem;text-align:center;font-family:'Cairo',sans-serif}
.remove-item{width:24px;height:24px;border-radius:6px;border:1px solid #fca5a5;background:#fef2f2;color:#dc2626;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:.8rem}
/* product selector */
.prod-picker{border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;max-height:240px;overflow-y:auto}
.prod-row{display:flex;align-items:center;justify-content:space-between;padding:.5rem .85rem;cursor:pointer;border-bottom:1px solid #f1f5f9;transition:background .1s}
.prod-row:hover{background:#eff6ff}
.prod-row:last-child{border-bottom:none}
.stock-ok{color:#16a34a;font-size:.74rem;font-weight:600}
.stock-low{color:#d97706;font-size:.74rem;font-weight:600}
.stock-out{color:#dc2626;font-size:.74rem;font-weight:600}
/* respond */
.resp-item{background:#f8fafc;border-radius:9px;padding:.55rem .85rem;margin-bottom:5px;border:1px solid #e2e8f0}
.resp-btns{display:flex;gap:4px}
.resp-btn{padding:2px 8px;border-radius:6px;font-size:.75rem;font-weight:600;border:none;cursor:pointer;font-family:'Cairo',sans-serif;transition:all .12s}
.rb-ok {background:#f0fdf4;color:#16a34a;border:1px solid #bbf7d0}
.rb-ok.active{background:#16a34a;color:#fff}
.rb-part{background:#fffbeb;color:#92400e;border:1px solid #fde68a}
.rb-part.active{background:#d97706;color:#fff}
.rb-no {background:#fef2f2;color:#dc2626;border:1px solid #fca5a5}
.rb-no.active{background:#dc2626;color:#fff}
</style>
</head>
<body>
...
<div class="content-body">
    ...
    $statItems = [
        ...
        [count(array_filter($orders_out, function($o){ return $o['status']==='sent'; })), 'بانتظار الرد', 'bi-hourglass-split', '#d97706','#fffbeb'],
        [count(array_filter($orders_out, function($o){ return $o['status']==='approved'; })), 'معتمدة', 'bi-check-circle', '#16a34a','#f0fdf4'],
        [count(array_filter($orders_out, function($o){ return $o['status']==='converted'; })), 'محوّلة لفواتير', 'bi-file-earmark-check', '#7c3aed','#f5f3ff'],
    ];
    ...
    <div class="ctrl-bar">
        ...
        <button class="btn btn-sm ms-auto" style="border-radius:9px;background:#1e3a8a;color:#fff;font-weight:600;font-size:.82rem"
    ...
    <div class="sec-card">
        <div class="sec-hdr">
            <span style="font-size:.88rem;font-weight:700;color:#1e293b">
                <i class="bi bi-send me-2 text-primary"></i>طلباتي لـ <?= htmlspecialchars($factoryName ?: 'المعمل') ?>
            </span>
        </div>
        <div class="table-responsive">
        <table>
            <thead><tr>
                <th>#</th><th>رقم الطلب</th><th>التاريخ</th><th>تاريخ التسليم المطلوب</th>
                <th>العملة</th><th>إجمالي الطلب</th><th>عدد البنود</th>
            ...
            <tr>
                ...
                <td class="fw-600" style="font-size:.84rem"><?=htmlspecialchars($ord['order_number'])?></td>
                ...
                <td class="fw-600" style="font-size:.83rem"><?=number_format($ord['total_amount'],2)?></td>
                <td class="text-center"><?=$items_cnt?></td>
                <td>
                    <span class="badge bg-<?=$sc[1]?>-subtle text-<?=$sc[1]?> border border-<?=$sc[1]?>-subtle"
                ...
                <td>
                    <div class="d-flex gap-1 justify-content-center">
                        <button class="act-btn" onclick="viewOrder(<?=$ord['id']?>)" title="عرض التفاصيل">
                            <i class="bi bi-eye"></i>
                        </button>
                        <?php if ($ord['status']==='draft'): ?>
                        <button class="act-btn success" onclick="sendOrder(<?=$ord['id']?>, '<?=htmlspecialchars($ord['order_number'],ENT_QUOTES)?>')" title="إرسال للمعمل">
                            <i class="bi bi-send"></i>
                        </button>
                        <?php endif; ?>
                        <?php if (in_array($ord['status'],['approved','partially_approved']) && !$ord['purchase_id']): ?>
                        <button class="act-btn success" onclick="convertOrder(<?=$ord['id']?>, '<?=htmlspecialchars($ord['order_number'],ENT_QUOTES)?>')" title="تحويل لفاتورة شراء">
    ...
    <div class="sec-card">
        ...
        <table>
            <thead><tr>
                <th>#</th><th>رقم الطلب</th><th>من فرع</th><th>التاريخ</th>
                <th>التسليم المطلوب</th><th>الإجمالي</th><th>الحالة</th>
            ...
            <tr>
                ...
                <td class="text-muted small"><?=$ord['order_date']?></td>
                <td class="text-muted small"><?=$ord['required_date'] ?? '—'?></td>
                <td class="fw-600" style="font-size:.83rem"><?=number_format($ord['total_amount'],2)?> <?=$ord['currency']?></td>
                <td>
                    <span class="badge bg-<?=$sc[1]?>-subtle text-<?=$sc[1]?> border border-<?=$sc[1]?>-subtle"
                          style="font-size:.72rem">
                        <i class="bi <?=$sc[2]?> me-1"></i><?=$sc[0]?>
                ...
                <td>
                    <div class="d-flex gap-1 justify-content-center">
                        ...
                        <button class="act-btn success" onclick="openRespondModal(<?=$ord['id']?>)" title="الرد على الطلب">
  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      ...
      <div class="modal-body px-4 pt-3 pb-2">
        <div class="row g-3">
          ...
          <div class="col-md-2">
            ...
            <input type="number" id="reqExRate" class="form-control form-control-sm" value="1" step="0.0001">
          </div>
          <div class="col-md-5">
          ...
          <div class="col-md-5">
            <label class="form-label small fw-600 text-secondary mb-1">
              <i class="bi bi-search me-1"></i>اختر من منتجات <?= htmlspecialchars($factoryName ?: 'المعمل') ?>
            </label>
      ...
      <div class="modal-footer border-0 px-4 pb-3 pt-1">
        ...
        <button class="btn btn-sm fw-600" style="border-radius:8px;min-width:130px;background:#1e3a8a;color:#fff" onclick="submitOrder(false)">
          <i class="bi bi-save me-1"></i>حفظ كمسودة
        </button>
        <button class="btn btn-sm fw-600" style="border-radius:8px;min-width:130px;background:#16a34a;color:#fff" onclick="submitOrder(true)">
    ...
    <div class="modal-content">
      <div class="modal-header mhdr-in py-3 px-4 border-0">
        <div>
          ...
          <div id="respOrderNum" style="font-size:.78rem;color:rgba(255,255,255,.75);margin-top:2px">—</div>
        </div>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body px-4 pt-3 pb-2">
        ...
        <div class="mt-3">
          ...
          <textarea id="respNotes" class="form-control form-control-sm" rows="2" placeholder="اختياري"></textarea>
        </div>
      </div>
      <div class="modal-footer border-0 px-4 pb-3 pt-1">
        <button type="button" class="btn btn-sm btn-light" data-bs-dismiss="modal" style="border-radius:8px">إلغاء</button>
        <button class="btn btn-sm fw-600" style="border-radius:8px;min-width:110px;background:#dc2626;color:#fff" onclick="respondOrder('rejected')">
          <i class="bi bi-x-circle me-1"></i>رفض الكل
        </button>
        <button class="btn btn-sm fw-600" style="border-radius:8px;min-width:110px;background:#16a34a;color:#fff" onclick="respondOrder('approved')">
...
function viewOrder(id) {
    ...
    post({_action:'get_order', id}).then(d => {
        ...
        const ord = d.data;
        const statusMap = <?= json_encode(array_map(function($s){ return $s[0]; }, $statusConfig)) ?>;
