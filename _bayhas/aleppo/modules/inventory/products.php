    ...
    try {
        ...
        if ($act === 'get_product_detail') {
            ...
            foreach ($sizes as $s) {
                ...
                if (!isset($grpMap[$key])) $grpMap[$key] = ['sizes'=>[],'selling_price'=>$s['selling_price'],'packet_qty'=>0];
        ...
        elseif ($act === 'save_product') {
            ...
            if (!$name || !$model) throw new Exception('الاسم ورقم الموديل مطلوبان');
            ...
            if ($wh_id) {
                ...
                foreach ($vr_stmt->fetchAll() as $vr) {
                    $pdo->prepare("INSERT IGNORE INTO `{$TWI}` (warehouse_id,variant_id,product_id,quantity,avg_cost_usd)
                        VALUES (?,?,?,0,0)")
...
$in_stock = count(array_filter($products, function($p){ return $p['total_stock'] > 0; }));
...
foreach ($warehouses as $wh) {
    $st = $pdo->prepare("SELECT COUNT(DISTINCT product_id) AS items,
        ...
        COALESCE(SUM(quantity*avg_cost_usd),0) AS val
...
$root_cats = array_filter($categories, function($c){ return !$c['parent_id']; });
...
.cat-chip{padding:5px 14px;border-radius:20px;font-size:.8rem;font-weight:600;cursor:pointer;border:1px solid #e2e8f0;background:#fff;color:#64748b;transition:all .15s;text-decoration:none;display:inline-block}
.cat-chip:hover{border-color:#3b82f6;color:#1d4ed8}
.cat-chip.act-final{background:#E6F1FB;color:#0C447C;border-color:#85B7EB}
.cat-chip.act-cons{background:#FAEEDA;color:#633806;border-color:#EF9F27}
.cat-chip.act-all{background:#1e3a8a;color:#fff;border-color:#1e3a8a}
...
.prod-thumb{width:32px;height:32px;border-radius:9px;background:#f1f5f9;border:1px solid #e2e8f0;display:flex;align-items:center;justify-content:center;font-size:1rem;color:#94a3b8;flex-shrink:0}
...
.wh-card{background:#fff;border-radius:14px;border:1px solid #e2e8f0;overflow:hidden}
.wh-hdr{padding:.75rem 1.1rem;border-bottom:1px solid #f1f5f9;display:flex;align-items:center;gap:.65rem}
.wh-icon{width:36px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:1.1rem;flex-shrink:0}
.wh-stat-row{display:flex;justify-content:space-between;font-size:.81rem;padding:.45rem 1.1rem;border-bottom:1px solid #f1f5f9}
.wh-stat-row:last-child{border-bottom:none}
/* مودال */
.modal-content{border-radius:16px;border:none}
.mhdr{border-radius:16px 16px 0 0;border:none}
.variant-row{background:#f8fafc;border-radius:9px;padding:.45rem .75rem;margin-bottom:4px;display:flex;align-items:center;gap:.5rem}
.size-tag{display:inline-flex;align-items:center;justify-content:center;min-width:28px;padding:2px 6px;border-radius:6px;font-size:.75rem;font-weight:600;background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe}
.color-dot{width:12px;height:12px;border-radius:50%;border:1px solid rgba(0,0,0,.15);flex-shrink:0}
.stock-ok{color:#16a34a;font-weight:600}
.stock-low{color:#d97706;font-weight:600}
.stock-out{color:#dc2626;font-weight:600}
.n{font-variant-numeric:tabular-nums}
.clr-dot{width:14px;height:14px;border-radius:50%;border:1px solid rgba(0,0,0,.12);flex-shrink:0;display:inline-block}
.grp-tag{display:inline-flex;align-items:center;gap:3px;background:#eff6ff;color:#1e3a8a;border:1px solid #bfdbfe;border-radius:12px;font-size:.68rem;padding:1px 6px;font-weight:600;white-space:nowrap}
.grp-tag.g2{background:#f0fdf4;color:#065f46;border-color:#bbf7d0}
.grp-tag.g3{background:#fff7ed;color:#7c2d12;border-color:#fed7aa}
.grp-tag.g4{background:#f5f3ff;color:#4c1d95;border-color:#ddd6fe}
.det-sec{border-radius:10px;border:1px solid #f1f5f9;overflow:hidden;margin-bottom:10px}
.det-sec-hdr{background:#f8fafc;padding:7px 12px;font-size:.75rem;font-weight:700;color:#1e293b;border-bottom:1px solid #f1f5f9;display:flex;align-items:center;gap:6px}
.det-sec-body{padding:10px 12px}
.det-row{display:flex;justify-content:space-between;font-size:.78rem;padding:3px 0;border-bottom:1px solid #f8fafc}
.det-row:last-child{border-bottom:none}
.det-row span:first-child{color:#64748b}
.det-row span:last-child{font-weight:600;color:#1e293b}
.grp-block{background:#f8fafc;border-radius:8px;padding:8px 10px;margin-bottom:6px}
.sz-mini{display:inline-flex;align-items:center;justify-content:center;width:26px;height:24px;border-radius:5px;background:#fff;border:1px solid #e2e8f0;font-size:.7rem;font-weight:600;color:#334155}
</style>
</head>
<body>
...
<div class="content-body">
    ...
    <div class="d-flex align-items-center gap-2 mb-3 flex-wrap">
        ...
        <?php foreach ($root_cats as $cat):
            ...
            $cnt = count(array_filter($products, function($p) use($cat){ return $p['cat_parent'] == $cat['id'] || $p['category_id'] == $cat['id']; }));
    ...
    <div class="sec-card mb-3">
        <div class="sec-hdr">
            <span style="font-size:.88rem;font-weight:700;color:#1e293b">
                ...
                <span class="text-muted fw-400" style="font-size:.8rem">
                    — <?= htmlspecialchars(array_filter($categories, function($c) use($sel_cat){ return $c['id']==$sel_cat; })[array_key_first(array_filter($categories, function($c) use($sel_cat){ return $c['id']==$sel_cat; }))] ['name'] ?? '') ?>
                </span>
                <?php endif; ?>
            </span>
        ...
        <table>
            ...
            <?php foreach ($products as $i => $prod):
                ...
                try {
                    ...
                    foreach ($pSizes as $s) {
                        ...
                        $grps[$k][] = $s['size'];
                        if ($minSell === null || (float)$s['selling_price'] < $minSell) $minSell = (float)$s['selling_price'];
                    }
                } catch (Exception $e) { $pSizes = []; }
            ...
            <tr id="pRow_<?=$prod['id']?>">
                ...
                <!-- اسم المنتج -->
                <td>
                    <div class="fw-600" style="font-size:.84rem"><?=htmlspecialchars($prod['name'])?></div>
                    <div class="text-muted" style="font-size:.7rem"><?=htmlspecialchars($prod['fabric_type']??'')?></div>
                </td>

                <!-- الموديل -->
                <td class="n text-muted" style="font-size:.78rem;direction:ltr"><?=htmlspecialchars($prod['model_number'])?></td>

                <!-- الفئة -->
                <td>
                    <span class="badge" style="background:<?=$cc[0]?>;color:<?=$cc[1]?>;border:1px solid <?=$cc[2]?>;border-radius:20px;font-size:.68rem;padding:2px 7px">
                ...
                <?php else: $gi=0; foreach ($grps as $price => $sizes): $gcls=['','g2','g3','g4'][$gi]??''; ?>
                ...
                <td>
                    ...
                    <?php foreach (array_slice($pColors,0,6) as $clr): ?>
                        <span class="clr-dot" style="background:<?=htmlspecialchars($clr['hex_code'])?>" title="<?=htmlspecialchars($clr['name'])?>"></span>
                    <?php endforeach; ?>
                ...
                <td>
                    <div class="d-flex gap-1 justify-content-center">
                        ...
                        <button class="act-btn <?=$prod['is_active']?'danger':''?>"
                            ...
                            <i class="bi bi-<?=$prod['is_active']?'slash-circle':'check-circle'?>"></i>
                        </button>
                    </div>
                </td>
            </tr>
        ...
        <div class="wh-card">
            <div class="wh-hdr">
                ...
                <div>
                    ...
                    <div class="text-muted" style="font-size:.73rem"><?=htmlspecialchars($wh['code'])?></div>
                </div>
            ...
            <div class="wh-stat-row">
                ...
                <span class="n fw-600" style="color:<?=$wClr?>"><?=number_format($ws['val'],2)?> $</span>
            </div>
        </div>
    </div>
    <?php endforeach; ?>
    ...
    <div class="modal-content">
      <div class="modal-header mhdr py-3 px-4 border-0"
        ...
        <div>
          ...
          <div id="mSub" style="font-size:.78rem;color:rgba(255,255,255,.75);margin-top:2px">فرع البيع</div>
        </div>
      ...
      <div class="modal-body px-4 pt-3 pb-2">
        ...
        <div class="row g-2 mb-3">
          <div class="col-md-4">
            <label class="form-label small fw-600 text-secondary mb-1">التصنيف <span class="text-danger">*</span></label>
          ...
          <div class="col-md-4">
            ...
            <input type="number" id="mPacket" class="form-control form-control-sm" placeholder="مثلاً: 12" min="1">
          </div>
        </div>
        ...
        <div class="row g-2 mb-3">
          <div class="col-md-6">
            <label class="form-label small fw-600 text-secondary mb-1">اسم المنتج <span class="text-danger">*</span></label>
            <input type="text" id="mName" class="form-control form-control-sm" placeholder="بنطلون جينز أطفال">
          </div>
          <div class="col-md-6">
            <label class="form-label small fw-600 text-secondary mb-1">رقم الموديل <span class="text-danger">*</span></label>
            <input type="text" id="mModel" class="form-control form-control-sm" placeholder="JNS-001" dir="ltr">
          </div>
        </div>
        ...
        <div id="mAgeSection" class="row g-2 mb-3" style="display:none!important">
          ...
          <div class="col-3">
            <input type="number" id="mAgeFrom" class="form-control form-control-sm" placeholder="من" min="0">
          </div>
          <div class="col-3">
            <input type="number" id="mAgeTo" class="form-control form-control-sm" placeholder="إلى" min="0">
          </div>
          <div class="col-3">
          ...
          <div class="col-3">
            <input type="number" id="mAgeStep" class="form-control form-control-sm" placeholder="خطوة" value="1" min="1">
          </div>
        </div>
        ...
        <div id="mSizesSection" class="mb-3" style="display:none!important">
          <div class="d-flex align-items-center justify-content-between mb-2">
            ...
            <button type="button" class="btn btn-sm btn-outline-primary" style="border-radius:8px;font-size:.77rem"
                    onclick="addSize()">
      ...
      <div class="modal-footer border-0 px-4 pb-3 pt-1">
        ...
        <button type="button" class="btn btn-sm fw-600" style="border-radius:8px;min-width:110px;background:#1e3a8a;color:#fff"
                onclick="saveProduct()" id="mSaveBtn">
    ...
    <div class="modal-content">
      <div class="modal-header py-3 px-4 border-0"
        ...
        <div>
          ...
          <div style="font-size:.78rem;color:rgba(255,255,255,.75);margin-top:2px">المتغيرات والمخزون</div>
        </div>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
...
const CAT_FINAL = <?= json_encode(array_values(array_filter($categories, function($c){ return !$c['parent_id'] && $c['id']==1; }))) ?>;
...
function viewDetail(id) {
    ...
    document.getElementById('detEditBtn').href      = 'product_add.php?id=' + id;
