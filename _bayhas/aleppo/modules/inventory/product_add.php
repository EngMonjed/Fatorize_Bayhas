...
if ($editId) {
    ...
    if ($editProd) {
        ...
        // جلب الألوان المميزة عبر color_id من جدول الألوان
        $clSt = $pdo->prepare("
        ...
        // بناء الكروبات: تجميع المقاسات بنفس selling_price
        $grpMap = [];
...
<title><?= $editProd ? 'تعديل منتج' : 'إضافة منتج' ?> — <?= htmlspecialchars($branchName) ?></title>
...
<header class="topbar">
    ...
    <nav class="ms-auto d-flex align-items-center gap-1" style="font-size:.78rem;color:#94a3b8">
        ...
        <span class="text-primary"><?= $editProd ? 'تعديل منتج' : 'إضافة منتج' ?></span>
    </nav>
</header>
...
<div class="content-body">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
        <div style="font-size:.95rem;font-weight:700;color:#1e293b">
            <i class="bi bi-<?= $editProd ? 'pencil-square' : 'plus-circle' ?> me-2 text-primary"></i>
            <?= $editProd ? 'تعديل منتج: ' . htmlspecialchars($editProd['name']) : 'إضافة منتج جديد' ?>
        </div>
    ...
    <input type="hidden" id="pId" value="<?= $editProd ? $editProd['id'] : '' ?>">
                        ...
                        <div class="d-flex gap-2">
                            <select id="pCat" class="form-select form-select-sm" style="flex:1">
                                ...
                                <option value="<?= $cat['id'] ?>"
                                    ...
                                    <?= ($editProd && $editProd['category_id'] == $cat['id']) ? 'selected' : '' ?>>
...
// ── تهيئة ──
<?php if ($editProd && !empty($editSizeGroups)): ?>
// تحميل بيانات التعديل
sizeGroups   = <?= json_encode(array_values($editSizeGroups)) ?>;
activeGrpIdx = sizeGroups.length - 1;
pricing      = <?= json_encode($editPricing) ?>;
// ربط مفاتيح الكروبات بالتسعير
pricing.forEach((p,i) => { if (sizeGroups[i]) p.group_key = sizeGroups[i].key; });
<?php endif; ?>

<?php if ($editProd && !empty($editColors)): ?>
// تحميل ألوان التعديل
(function(){
    const editClrs = <?= json_encode(array_map(fn($c) => ['id'=>(int)($c['id']??0),'name'=>$c['name'],'hex'=>$c['hex']??$c['hex_code']??'#ccc'], $editColors)) ?>;
    selColorIdxs = [];
    editClrs.forEach(ec => {
        const idx = allColors.findIndex(ac => ac.name === ec.name);
        if (idx >= 0) { selColorIdxs.push(idx); selColors.push({name:ec.name, hex:ec.hex}); }
        else { // لون غير موجود في القائمة — نضيفه مؤقتاً
            allColors.push({id:0, name:ec.name, hex:ec.hex});
            selColorIdxs.push(allColors.length - 1);
            selColors.push({name:ec.name, hex:ec.hex});
        }
    });
})();
<?php endif; ?>

<?php if ($editProd && $editProd['supplier_id']): ?>
// ضبط المورد
(function(){
    const spSel = document.getElementById('pSupplier');
    if (spSel) { spSel.value = '<?= (int)$editProd['supplier_id'] ?>'; onSupplierChange(); }
})();
<?php endif; ?>
