 ...
 * المسار: /bayhas/aleppo/modules/inventory/consumable_issues.php
 */
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['_action'])) {
    ...
    try {
        ...
        if ($act === 'save_issue') {
            ...
            if (!$dept)       throw new Exception('يجب تحديد الجهة المستلمة');
        ...
        elseif ($act === 'confirm_issue') {
            ...
            $iSt->execute([$id]);
            $iss = $iSt->fetch();
            if (!$iss)                       throw new Exception('أمر الصرف غير موجود');
            if ($iss['status'] !== 'draft')  throw new Exception('يمكن تأكيد المسودات فقط');
            ...
            try {
                ...
                foreach ($items->fetchAll() as $row) {
                    ...
                    $pdo->prepare("INSERT INTO `{$TM}`
                        ...
                        VALUES (?,?,?,'issue','out',?,?,?,?,?,?,?,?,1,?)")
                        ->execute([
                            $movNo,$row['item_id'],$iss['warehouse_id'],
                            $row['quantity'],$st['avg_cost_usd']??0,$row['total_cost_usd'],
                            $avail,$avail-$row['quantity'],
                            'issue',$id,$iss['issue_date'],$_SESSION['user_id']
                        ]);
        ...
        elseif ($act === 'cancel_issue') {
            ...
            $iSt->execute([$id]);
            $iss = $iSt->fetch();
            if (!$iss)                           throw new Exception('غير موجود');
            if ($iss['status'] === 'cancelled')  throw new Exception('ملغى مسبقاً');
...
if ($search)  { $where.=' AND (i.issue_no LIKE ? OR i.department LIKE ?)'; $params[]="%{$search}%"; $params[]="%{$search}%"; }
...
/* بنود الصرف */
.issue-line{display:grid;grid-template-columns:minmax(0,2fr) 80px 80px minmax(0,1.5fr) 28px;gap:6px;align-items:center;background:#f8fafc;border-radius:8px;padding:6px 8px;margin-bottom:5px}
.issue-line input,.issue-line select{font-size:.78rem;padding:4px 6px;border:1px solid #e2e8f0;border-radius:6px;width:100%;background:#fff}
.issue-line input.stock{background:#f0fdf4;color:#16a34a;border-color:#bbf7d0;font-size:.72rem}
...
.dept-chip{display:inline-block;background:#eff6ff;color:#1e3a8a;border:1px solid #bfdbfe;border-radius:20px;font-size:.72rem;padding:2px 10px;cursor:pointer;margin:2px;transition:all .1s}
.dept-chip:hover{background:#1e3a8a;color:#fff}
</style>
</head>
<body>
...
<ul class="nav nav-tabs mb-3" style="border-bottom:2px solid #e2e8f0">
    <li class="nav-item"><a class="nav-link fw-600" href="consumables.php" style="border:none;color:#64748b;font-size:.83rem"><i class="bi bi-box-seam me-1"></i>المواد</a></li>
    ...
    <li class="nav-item"><a class="nav-link fw-600 active" href="#" style="border:none;border-bottom:2px solid #f59e0b;color:#f59e0b;font-size:.83rem;margin-bottom:-2px"><i class="bi bi-arrow-bar-up me-1"></i>أوامر الصرف</a></li>
</ul>
...
<div class="row g-3 mb-4">
    <div class="col-6 col-md-3">
        <div class="stat-card">
            <div class="stat-icon" style="background:#fef3c7"><i class="bi bi-arrow-bar-up text-warning"></i></div>
            <div><div class="stat-val"><?= $stats['total'] ?></div><div class="stat-lbl">إجمالي أوامر الصرف</div></div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="stat-card">
            <div class="stat-icon" style="background:#f1f5f9"><i class="bi bi-hourglass text-secondary"></i></div>
    ...
    <div class="col-6 col-md-3">
        <div class="stat-card">
            ...
            <div><div class="stat-val"><?= $stats['confirmed'] ?></div><div class="stat-lbl">مصروفة</div></div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="stat-card">
            <div class="stat-icon" style="background:#fee2e2"><i class="bi bi-currency-dollar text-danger"></i></div>
            <div><div class="stat-val n">$ <?= number_format($stats['total_cost'],2) ?></div><div class="stat-lbl">إجمالي التكاليف</div></div>
        </div>
    </div>
</div>
    ...
    <div class="tbl-hdr">
        <form method="get" class="d-flex gap-2 flex-wrap align-items-center w-100">
            <input type="text" name="q" value="<?= htmlspecialchars($search) ?>"
                   placeholder="رقم الأمر أو الجهة..."
            ...
            <?php if ($search||$statusF): ?><a href="consumable_issues.php" class="btn btn-sm btn-light" style="border-radius:8px"><i class="bi bi-x-lg"></i></a><?php endif; ?>
        </form>
        <button class="btn btn-sm fw-600" style="border-radius:9px;background:#f59e0b;color:#fff;font-size:.82rem;border:none"
...
<div class="tbl-wrap">
    ...
    <table class="mtbl">
        <thead><tr>
            <th>رقم الأمر</th><th>التاريخ</th><th>المستودع</th>
            <th>الجهة المستلمة</th><th>المواد</th><th>التكلفة ($)</th>
        ...
        ?>
        <tr>
            <td class="n fw-600" style="direction:ltr;color:#f59e0b"><?= htmlspecialchars($iss['issue_no']) ?></td>
            ...
            <td>
                <span class="badge bg-primary-subtle text-primary" style="font-size:.75rem">
                    <i class="bi bi-building me-1"></i><?= htmlspecialchars($iss['department']??'—') ?>
                </span>
            </td>
            <td class="text-center"><span class="badge bg-secondary-subtle text-secondary"><?= $iss['items_count'] ?> مادة</span></td>
            ...
            <td>
                <div class="d-flex gap-1 justify-content-center">
                    ...
                    <button class="act-btn success-h" onclick="confirmIssue(<?=$iss['id']?>, '<?=htmlspecialchars($iss['issue_no'],ENT_QUOTES)?>')" title="تأكيد الصرف">
                        <i class="bi bi-check-circle"></i>
                    </button>
                    <?php endif; ?>
                    <?php if ($iss['status']!=='cancelled'): ?>
                    <button class="act-btn danger" onclick="cancelIssue(<?=$iss['id']?>, '<?=htmlspecialchars($iss['issue_no'],ENT_QUOTES)?>')" title="إلغاء">
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content" style="border-radius:16px;border:none">
      <div class="modal-header py-3 px-4 border-0"
        ...
        <h6 class="modal-title text-white fw-700 mb-0"><i class="bi bi-arrow-bar-up me-2"></i>أمر صرف مستهلكات جديد</h6>
      ...
      <div class="modal-body px-4 pt-3">
        ...
        <div class="row g-3 mb-3 pb-3" style="border-bottom:1px solid #f1f5f9">
          ...
          <div class="col-md-4">
            ...
            <input type="text" id="iDept" class="form-control form-control-sm" placeholder="مثال: فرع البيع، مستودع الخياطة...">
            <div class="mt-1">
                <?php foreach ($DEPT_SUGGESTIONS as $d): ?>
                <span class="dept-chip" onclick="document.getElementById('iDept').value='<?= htmlspecialchars($d) ?>'"><?= $d ?></span>
                <?php endforeach; ?>
            </div>
          </div>
          <div class="col-md-4">
        ...
        <div class="d-flex align-items-center justify-content-between mb-2">
          ...
          <button class="btn btn-sm" style="border-radius:8px;border:1px solid #f59e0b;color:#b45309;font-size:.76rem"
      ...
      <div class="modal-footer border-0 px-4 pb-4">
        ...
        <button class="btn btn-sm fw-600" style="border-radius:8px;background:#f59e0b;color:#fff;min-width:120px;border:none"
...
function saveIssue(){
    ...
    post({_action:'save_issue',warehouse_id:whId,department:dept,
