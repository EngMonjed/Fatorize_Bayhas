 ...
 * المسار: /bayhas/aleppo/modules/inventory/movements.php
 */
session_start();
...
if ($tab === 'consumables') {
    ...
    if ($typeF)  { $where.=' AND m.movement_type=?'; $params[]=$typeF; }
...
if ($tab === 'products') {
    ...
    if ($whF)    { $whereP.=' AND pi.warehouse_id=?'; $paramsP[]=$whF; }
    if ($search) { $whereP.=' AND pi.product_name LIKE ?'; $paramsP[]="%{$search}%"; }
    ...
    if ($whF)    { $whereS.=' AND si.warehouse_id=?'; $paramsS[]=$whF; }
    if ($search) { $whereS.=' AND si.item_name LIKE ?'; $paramsS[]="%{$search}%"; }
    ...
    usort($prodMovements, function($a,$b){ return strcmp($b['movement_date'],$a['movement_date']); });

    // إحصائيات المنتجات
    $prodIn  = array_filter($prodMovements, function($r){ return $r['direction']==='in'; });
    $prodOut = array_filter($prodMovements, function($r){ return $r['direction']==='out'; });
    $prodStats = [
...
table.mtbl{width:100%;border-collapse:collapse;font-size:.8rem}
table.mtbl th{background:#f8fafc;padding:7px 10px;font-weight:600;color:#64748b;font-size:.71rem;border-bottom:1px solid #f1f5f9;white-space:nowrap}
table.mtbl td{padding:7px 10px;border-bottom:1px solid #f8fafc;vertical-align:middle}
...
<ul class="nav nav-tabs mb-3" style="border-bottom:2px solid #e2e8f0">
    <li class="nav-item">
        <a class="nav-link fw-600 <?= $tab==='consumables'?'active':'' ?>" href="?tab=consumables&from=<?=$dateFrom?>&to=<?=$dateTo?>"
    ...
    <li class="nav-item">
        <a class="nav-link fw-600 <?= $tab==='products'?'active':'' ?>" href="?tab=products&from=<?=$dateFrom?>&to=<?=$dateTo?>"
...
<div class="row g-3 mb-4">
    <div class="col-6 col-md-3">
        <div class="stat-card">
            <div class="stat-icon" style="background:#eff6ff"><i class="bi bi-arrow-left-right text-primary"></i></div>
            <div><div class="stat-val"><?= $stats['total']??0 ?></div><div class="stat-lbl">إجمالي الحركات</div></div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="stat-card">
            <div class="stat-icon" style="background:#f0fdf4"><i class="bi bi-box-arrow-in-down text-success"></i></div>
            <div><div class="stat-val dir-in"><?= number_format($stats['total_in']??0,2) ?></div><div class="stat-lbl">إجمالي الوارد</div></div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="stat-card">
            <div class="stat-icon" style="background:#fee2e2"><i class="bi bi-box-arrow-up text-danger"></i></div>
            <div><div class="stat-val dir-out"><?= number_format($stats['total_out']??0,2) ?></div><div class="stat-lbl">إجمالي الصادر</div></div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="stat-card">
            <div class="stat-icon" style="background:#fef3c7"><i class="bi bi-currency-dollar text-warning"></i></div>
            <div>
                <div class="stat-val n" style="font-size:.9rem">
                    <span class="dir-in">+$ <?= number_format($stats['cost_in']??0,2) ?></span>
                    <span style="font-size:.75rem;color:#94a3b8;display:block">-$ <?= number_format($stats['cost_out']??0,2) ?></span>
                </div>
    ...
    <div class="tbl-hdr">
        <form method="get" class="d-flex gap-2 flex-wrap align-items-center w-100">
            ...
            <input type="text" name="q" value="<?= htmlspecialchars($search) ?>"
                   placeholder="بحث بالاسم أو المرجع..." class="form-control form-control-sm" style="width:180px;border-radius:8px">
            <select name="wh" class="form-select form-select-sm" style="width:150px;border-radius:8px" onchange="this.form.submit()">
                <option value="">كل المستودعات</option>
                <?php foreach ($warehouses as $wh): ?>
                <option value="<?=$wh['id']?>" <?= $whF==$wh['id']?'selected':'' ?>><?= htmlspecialchars($wh['name']) ?></option>
                <?php endforeach; ?>
            </select>
            <?php if ($tab==='consumables'): ?>
            <select name="type" class="form-select form-select-sm" style="width:120px;border-radius:8px" onchange="this.form.submit()">
                <option value="">كل الأنواع</option>
                <?php foreach (['receive'=>'استلام','issue'=>'صرف','return_in'=>'إرجاع','adjust'=>'تسوية','waste'=>'هالك'] as $k=>$v): ?>
            ...
            <a href="?tab=<?=$tab?>&from=<?=$dateFrom?>&to=<?=$dateTo?>" class="btn btn-sm btn-light" style="border-radius:8px"><i class="bi bi-x-lg"></i></a>
            <?php endif; ?>
        </form>
    </div>
</div>
...
<div class="tbl-wrap">
    ...
    <table class="mtbl">
        <thead><tr>
            <th>رقم الحركة</th><th>التاريخ</th><th>المادة</th><th>المستودع</th>
            <th>النوع</th><th>الاتجاه</th>
            <th class="text-center">الكمية</th>
            <th class="text-center">قبل</th><th class="text-center">بعد</th>
        ...
        <tr><td colspan="12" class="text-center text-muted py-5">
            <i class="bi bi-arrow-left-right d-block mb-2" style="font-size:2rem;opacity:.2"></i>
        ...
        ?>
        <tr>
            <td dir="ltr" style="font-size:.72rem;color:#94a3b8"><?= htmlspecialchars($mov['movement_no']) ?></td>
            <td class="text-muted"><?= $mov['movement_date'] ?></td>
            <td>
                <div class="fw-600"><?= htmlspecialchars($mov['item_name']) ?></div>
                <div style="font-size:.7rem;color:#94a3b8"><?= htmlspecialchars($mov['unit']) ?></div>
            </td>
            <td style="font-size:.78rem"><?= htmlspecialchars($mov['wh_name']??'—') ?></td>
            <td><span class="badge <?=$mt['cls']?>" style="font-size:.68rem"><i class="bi <?=$mt['icon']?> me-1"></i><?=$mt['label']?></span></td>
            <td>
            ...
            <td class="n text-center text-muted" style="font-size:.75rem"><?= number_format($mov['qty_before'],3) ?></td>
            <td class="n text-center text-muted" style="font-size:.75rem"><?= number_format($mov['qty_after'],3) ?></td>
            <td class="n text-center" style="font-size:.75rem">$ <?= number_format($mov['unit_cost_usd'],4) ?></td>
    ...
    <table class="mtbl">
        <thead><tr>
            <th>التاريخ</th><th>العملية</th><th>المنتج</th>
            <th>القياس</th><th>اللون</th><th>المستودع</th>
            <th>المرجع</th><th>الطرف الآخر</th>
            <th class="text-center">الكمية</th>
        ...
        <tr>
            ...
            <td><span class="badge <?=$mt['cls']?>" style="font-size:.68rem"><i class="bi <?=$mt['icon']?> me-1"></i><?=$mt['label']?></span></td>
            <td>
                <div class="fw-600" style="font-size:.8rem"><?= htmlspecialchars($mov['product_name']) ?></div>
                <div style="font-size:.7rem;color:#94a3b8" dir="ltr"><?= htmlspecialchars($mov['model_number']??'') ?></div>
            </td>
            <td style="font-size:.78rem"><?= $mov['size']??'—' ?></td>
            <td style="font-size:.78rem"><?= $mov['color']??'—' ?></td>
            <td style="font-size:.78rem"><?= htmlspecialchars($mov['wh_name']??'—') ?></td>
            <td style="font-size:.72rem;color:#1e3a8a;font-weight:600" dir="ltr"><?= htmlspecialchars($mov['ref_no']??'—') ?></td>
