    ...
    try {
        ...
        elseif ($act === 'save_purchase') {
            ...
            $pdo->prepare("INSERT INTO `{$TP}`
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
                ->execute([
                    $invNo, $suppRef, $supplierId, $whId, $currency, $exRate,
                    $invDate, $payMethod, $notes,
                    $subtotalOrig, $subtotalUsd,
                    $discPct, $discOrig, $discUsd,
                    $taxPct, $taxOrig, $taxUsd,
                    $totalOrig, $totalUsd,
                    $paidOrig, $paidUsd,
                    $balanceOrig, $balanceUsd,
            ...
            foreach ($rows as $r) {
                ...
                $pdo->prepare("INSERT INTO `{$TPI}`
                    ...
                    ->execute([
                        ...
                        $unitPrOrig, $unitPrUsd,
                        $disc, $totalRowOrig, $totalRowUsd, $movId
                    ]);
            }

            echo json_encode(['ok' => true, 'id' => $purchaseId, 'invoice_no' => $invNo,
...
/* ── بنود الفاتورة ── */
.line-grid{display:grid;grid-template-columns:minmax(0,2.2fr) 70px 90px 95px 55px 85px 85px 28px;gap:5px;align-items:center}
.line-hdr-grid{display:grid;grid-template-columns:minmax(0,2.2fr) 70px 90px 95px 55px 85px 85px 28px;gap:5px}
.inv-row{background:#f8fafc;border-radius:8px;padding:5px 7px;margin-bottom:5px}
.inv-row input,.inv-row select{font-size:.78rem;padding:4px 6px;border:1px solid #e2e8f0;border-radius:6px;width:100%;background:#fff;color:#1e293b}
.inv-row input[readonly]{background:#f1f5f9;color:#64748b;border-color:#e2e8f0}
.inv-row input.calc{background:#f0fdf4;color:#16a34a;border-color:#86efac;font-weight:600}
.inv-row input.ref{background:#fef9ee;color:#92400e;border-color:#fcd34d;font-size:.72rem}
.del-btn{width:26px;height:26px;border-radius:6px;border:1px solid #fca5a5;background:#fff;color:#dc2626;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:.8rem;flex-shrink:0}
.tot-box{background:#f8fafc;border-radius:10px;padding:10px 14px}
.tot-row{display:flex;justify-content:space-between;font-size:.8rem;padding:3px 0}
.tot-row.final{font-weight:700;font-size:.9rem;border-top:1px solid #e2e8f0;padding-top:6px;margin-top:3px;color:#1e293b}
.n{font-variant-numeric:tabular-nums}
.legend{display:flex;gap:12px;font-size:.7rem;color:#64748b;margin-bottom:8px;flex-wrap:wrap}
...
<ul class="nav nav-tabs mb-3" style="border-bottom:2px solid #e2e8f0">
    ...
    <li class="nav-item"><a class="nav-link fw-600 active" href="#" style="border:none;border-bottom:2px solid #1e3a8a;color:#1e3a8a;font-size:.83rem;margin-bottom:-2px"><i class="bi bi-cart-plus me-1"></i>فواتير الشراء</a></li>
    ...
    <li class="nav-item"><a class="nav-link fw-600" href="consumable_sales.php" style="border:none;color:#64748b;font-size:.83rem"><i class="bi bi-receipt me-1"></i>فواتير البيع</a></li>
</ul>

<!-- إحصائيات -->
<div class="row g-3 mb-4">
    <div class="col-6 col-md-3">
        <div class="stat-card"><div class="stat-icon" style="background:#eff6ff"><i class="bi bi-receipt text-primary"></i></div>
        <div><div class="stat-val"><?= $stats['total'] ?></div><div class="stat-lbl">إجمالي الفواتير</div></div></div>
    </div>
    <div class="col-6 col-md-3">
        <div class="stat-card"><div class="stat-icon" style="background:#fef3c7"><i class="bi bi-hourglass-split text-warning"></i></div>
        <div><div class="stat-val"><?= ($stats['confirmed']??0)+($stats['partial']??0) ?></div><div class="stat-lbl">معلّقة / جزئية</div></div></div>
    </div>
    <div class="col-6 col-md-3">
        <div class="stat-card"><div class="stat-icon" style="background:#f0fdf4"><i class="bi bi-currency-dollar text-success"></i></div>
        <div><div class="stat-val n"><?= number_format($stats['total_amount'],2) ?> $</div><div class="stat-lbl">إجمالي المشتريات</div></div></div>
    </div>
    <div class="col-6 col-md-3">
        <div class="stat-card"><div class="stat-icon" style="background:#fee2e2"><i class="bi bi-exclamation-circle text-danger"></i></div>
        <div><div class="stat-val n"><?= number_format($stats['total_balance'],2) ?> $</div><div class="stat-lbl">إجمالي المستحق</div></div></div>
    </div>
</div>
...
<div class="tbl-wrap">
    <div class="tbl-hdr">
        <span style="font-size:.88rem;font-weight:700;color:#1e293b"><i class="bi bi-list-ul me-1 text-primary"></i>سجل فواتير الشراء</span>
    ...
    <table class="mtbl">
        <thead><tr>
            <th>رقم الفاتورة</th><th>التاريخ</th><th>المورد</th><th>المستودع</th>
            <th>العملة</th><th>الإجمالي</th><th>المدفوع</th><th>المتبقي</th>
        ...
        <tr>
            ...
            <td><span class="badge bg-secondary-subtle text-secondary"><?= $pur['currency_code'] ?></span></td>
            <td class="n fw-600"><?= number_format($tO,2) ?> <?=$sym?></td>
            <td class="n text-success"><?= number_format($pO,2) ?> <?=$sym?></td>
            <td class="n <?= $bO>0?'text-danger fw-600':'' ?>"><?= number_format($bO,2) ?> <?=$sym?></td>
            ...
            <td>
                <div class="d-flex gap-1 justify-content-center">
                    ...
                    <button class="act-btn success-h" onclick="confirmInvoice(<?=$pur['id']?>,'<?=htmlspecialchars($pur['invoice_no'],ENT_QUOTES)?>')" title="تأكيد">
    ...
    <div class="modal-content" style="border-radius:16px;border:none">
      <div class="modal-header py-3 px-4 border-0"
        ...
        <div>
          ...
          <div style="font-size:.75rem;color:rgba(255,255,255,.7);margin-top:2px">رقم الفاتورة يُولَّد تلقائياً</div>
        </div>
      ...
      <div class="modal-body px-4 pt-3">
        ...
        <div class="row g-3 mb-3 pb-3" style="border-bottom:1px solid #f1f5f9">
          <div class="col-md-3">
            ...
            <div class="d-flex gap-1">
              ...
              <button class="btn btn-sm" style="border-radius:7px;border:1px solid #1e3a8a;color:#1e3a8a;padding:4px 7px"
          ...
          <div class="col-md-2">
            ...
            <input type="text" id="iSupplierRef" class="form-control form-control-sm" dir="ltr" placeholder="اختياري">
          </div>
          <div class="col-md-2">
          ...
          <div class="col-md-2">
            <label class="field-lbl">المبلغ المدفوع <span id="paidCurLbl" style="color:#0891b2;font-size:.7rem"></span></label>
            <input type="number" id="iPaid" class="form-control form-control-sm"
                   min="0" step="0.01" value="" placeholder="0.00" dir="ltr" oninput="calcTotals()">
          </div>
          <div class="col-md-4">
        ...
        <div class="legend">
          <span><span class="legend-dot" style="background:#fef9ee;border:1px solid #fcd34d"></span>سعر أساسي مرجعي</span>
          <span><span class="legend-dot" style="background:#f0fdf4;border:1px solid #86efac"></span>محسوب تلقائياً</span>
          <span><span class="legend-dot" style="background:#fff;border:1px solid #e2e8f0"></span>إدخال يدوي</span>
        </div>
      ...
      <div class="modal-footer border-0 px-4 pb-4">
        ...
        <button class="btn btn-sm fw-600" style="border-radius:8px;background:#1e3a8a;color:#fff;min-width:130px"
      ...
      <div class="modal-body px-4 pt-3">
        <div class="row g-3">
          <div class="col-md-7">
            ...
            <input type="text" id="spName" class="form-control form-control-sm" placeholder="اسم الشركة أو المورد">
          </div>
          <div class="col-md-5">
