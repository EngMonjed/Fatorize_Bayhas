 ...
 *retail1/modules/inventory/product_add.php
 */
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['_action'])) {
    ...
    try {
        ...
        elseif ($act === 'get_last_product') {
            $lp = $pdo->query("SELECT * FROM `{$TP}` ORDER BY id DESC LIMIT 1")->fetch();
            if (!$lp)
                throw new Exception('لا يوجد منتج سابق');
            $sz = $pdo->prepare("SELECT * FROM `{$TSZ}` WHERE product_id=? ORDER BY sort_order");
            $sz->execute([$lp['id']]);
        ...
        elseif ($act === 'add_color') {
            ...
            if (!$name)
                throw new Exception('اسم اللون مطلوب');
            try {
        ...
        elseif ($act === 'save_product') {
            ...
            if (!$name || !$model)
                throw new Exception('اسم المنتج ورقم الموديل مطلوبان');
            if ($id) {
...
<head>
    ...
    <style>
        ...
        .upload-box.has-img .img-overlay {
            position: absolute;
            inset: 0;
        ...
        .upload-box.has-img:hover .img-overlay {
            background: rgba(0, 0, 0, .45)
        }

        .upload-box.has-img:hover .img-overlay span {
            opacity: 1
        }

        .upload-box.has-img .img-overlay span {
            opacity: 0;
            color: #fff;
            font-size: .72rem;
            ...
            <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
                ...
                <div class="d-flex gap-2">
                    <button class="btn btn-sm"
                        style="border-radius:8px;border:1px solid #1e3a8a;color:#1e3a8a;font-size:.78rem"
                    ...
                    <button class="btn btn-sm fw-600"
                        style="border-radius:8px;background:#1e3a8a;color:#fff;font-size:.78rem;min-width:100px"
            ...
            <div class="row g-3">
                ...
                <div class="col-lg-8">
                        ...
                        <div class="sec-body">
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" id="pFabric" class="form-control form-control-sm"
                                        placeholder="مثال: RAPID، كوتون، جينز" value="">
                                </div>
                            </div>
                            <div class="row g-3 mt-0">
                                <div class="col-md-8">
                                    ...
                                    <div class="d-flex gap-2">
                                        <select id="pSupplier" class="form-select form-select-sm" style="flex:1"
                                ...
                                <div class="col-md-4" id="supplierInfoWrap" style="display:none">
                                    ...
                                    <div
                                        style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:6px 10px;font-size:.74rem;color:#475569">
                        ...
                        <div class="sec-body">
                            <div class="row g-3 mb-3">
                                <div class="col-md-4">
                                    ...
                                    <select id="pAgeType" class="form-select form-select-sm"
                                ...
                                <div class="col-md-4">
                                    <label class="field-lbl">&nbsp;</label>
                                    <div>
                                        <button class="btn-add-sm" id="btnNewGrp" onclick="startNewGroup()"
                                            style="height:31px">
                            ...
                            <!-- شبكة الأرقام -->
                            <div id="sizeGridWrap">
                                <div id="sizeGridLabel"
                                    style="font-size:.72rem;font-weight:600;color:#64748b;margin-bottom:5px"></div>
                    ...
                    <div class="sec-card">
                        <div class="sec-hdr">
                            ...
                            <span class="sec-title"><i class="bi bi-upc-scan me-1 text-primary"></i>الباركودات المولّدة
                                تلقائياً</span>
                        </div>
                        <div class="sec-body">
                            <div class="field-hint mb-2">باركود واحد لكل لون — يتولّد تلقائياً من رقم الموديل + اسم
                                اللون</div>
                    ...
                    <div class="sec-card">
                        <div class="sec-hdr">
                            ...
                            <span class="sec-title"><i class="bi bi-currency-dollar me-1 text-primary"></i>التسعير (بحسب
                                الكروب)</span>
                        </div>
                        <div class="sec-body">
                            <div id="pricingTable">
                                <div class="text-muted" style="font-size:.78rem">اختر القياسات أولاً لظهور جدول التسعير
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="sec-card">
                        <div class="sec-hdr">
                            ...
                            <span class="sec-title"><i class="bi bi-info-circle me-1 text-primary"></i>تفاصيل
                                إضافية</span>
                        </div>
                        <div class="sec-body">
                            <div class="row g-3">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="number" id="pMinQty" class="form-control form-control-sm" min="0"
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="upload-box" id="uploadBox"
                                        ...
                                        <i class="bi bi-cloud-arrow-up d-block mb-1" style="font-size:1.4rem"
                                            id="uploadIcon"></i>
                                    ...
                                    <div class="img-toast" id="imgToast">
                                        ...
                                        <button onclick="removeImg()"
                                            style="background:none;border:none;color:#16a34a;margin-right:auto;cursor:pointer;font-size:.72rem;padding:0">
                                            <i class="bi bi-x-lg"></i> إزالة
                                        </button>
                                    </div>
                                    <input type="file" id="pImg" accept="image/*" style="display:none"
                                        onchange="onImgSelect(this)">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-lg-4">
                    <div style="position:sticky;top:70px">
                        <!-- أزرار العمل العائمة -->
                        <div
                            style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:12px 14px;margin-bottom:12px;display:flex;flex-direction:column;gap:8px">
                            <button class="btn btn-sm fw-600 w-100"
                                style="border-radius:8px;background:#1e3a8a;color:#fff;font-size:.82rem"
                                onclick="saveProduct()" id="btnSave2">
                                <span id="saveTxt2"><i class="bi bi-floppy me-1"></i>حفظ المنتج</span>
                                <span id="saveSpin2" class="spinner-border spinner-border-sm"
                                    style="display:none"></span>
                            </button>
                            <div class="d-flex gap-2">
                                <button class="btn btn-sm w-50"
                                    style="border-radius:8px;border:1px solid #1e3a8a;color:#1e3a8a;font-size:.77rem"
                                    onclick="copyLastProduct()">
                                    <i class="bi bi-copy me-1"></i>نسخ آخر منتج
                                </button>
                                <a href="products.php" class="btn btn-sm btn-light w-50"
                                    style="border-radius:8px;font-size:.77rem;text-align:center">
                                    <i class="bi bi-x-lg me-1"></i>إلغاء
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="sb-card">
                            ...
                            <div class="sb-body">
                                ...
                                <div class="sb-row" style="align-items:flex-start"><span>الباكيت</span><span
                                        id="sbPacket" style="text-align:left">—</span></div>
    ...
    <div class="mini-modal" id="catModal">
        <div class="mini-modal-box">
            <div class="mini-modal-hdr">
                ...
                <button onclick="hideCatModal()"
                    style="background:none;border:none;color:rgba(255,255,255,.7);font-size:1rem;cursor:pointer">
            ...
            <div class="p-3">
                ...
                <div class="d-flex gap-2 justify-content-end">
                    <button class="btn btn-sm btn-light" style="border-radius:8px"
                        onclick="hideCatModal()">إلغاء</button>
            ...
            <div class="p-3">
                <div class="mb-3">
                    ...
                    <input type="text" id="colorName" class="form-control form-control-sm"
                        placeholder="مثال: أخضر داكن">
                </div>
                <div class="mb-3">
                    <label class="field-lbl">الكود اللوني</label>
                    <div class="d-flex gap-2 align-items-center">
                        <input type="color" id="colorHex" value="#000000"
                            style="width:44px;height:34px;border-radius:7px;border:1px solid #e2e8f0;padding:2px;cursor:pointer">
                        <input type="text" id="colorHexTxt" class="form-control form-control-sm" dir="ltr"
                            value="#000000" oninput="document.getElementById('colorHex').value=this.value"
                            placeholder="#rrggbb">
                    </div>
                </div>
                <div class="d-flex gap-2 justify-content-end">
                    <button class="btn btn-sm btn-light" style="border-radius:8px"
                        onclick="hideColorModal()">إلغاء</button>
                    <button class="btn btn-sm fw-600" style="border-radius:8px;background:#1e3a8a;color:#fff"
                        onclick="saveColor()">
                        <i class="bi bi-plus-lg me-1"></i>إضافة
                    </button>
    ...
    <div class="mini-modal" id="supplierModal">
        <div class="mini-modal-box" style="width:460px">
            <div class="mini-modal-hdr">
                ...
                <button onclick="hideSupplierModal()"
                    style="background:none;border:none;color:rgba(255,255,255,.7);font-size:1rem;cursor:pointer">
                    <i class="bi bi-x-lg"></i>
                </button>
            </div>
            <div class="p-3">
                ...
                <div class="row g-2 mb-2">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" id="spPhone" class="form-control form-control-sm" placeholder="+963..."
                            dir="ltr">
                    </div>
                </div>
                <div class="mb-3">
                ...
                <div class="d-flex gap-2 justify-content-end">
                    <button class="btn btn-sm btn-light" style="border-radius:8px"
                        onclick="hideSupplierModal()">إلغاء</button>
                    <button class="btn btn-sm fw-600" style="border-radius:8px;background:#1e3a8a;color:#fff"
